...
Time is a **uniform integer tick axis** measured from the project `epoch`, with no gaps and no compression. Every tick exists whether or not anyone is working. `Project.ticksPerDay` declares the resolution — 24 (hourly, the default), 8 (shift), 1 (daily), or finer such as 96 (quarter-hour). In a zone that changes its clock only 24 and finer are available, because a local day is 23 or 25 hours across a transition and coarser ticks stop aligning with local midnight; the axis refuses the combination when it is defined rather than months later inside a calendar. Precedence, resource constraints, deadlines, actuals, and schedule starts are all expressed on this one axis, so any two entities are directly comparable regardless of which calendar governs them. See ADR-8 in `05-technology-decisions.md` for why the axis is not a per-calendar working-time index.
...
`planningHorizon` bounds calendar materialisation and is derived when absent, by growing until every effective calendar can supply the **network's longest chain** of durations and positive lags. That is what the critical path needs and no more. Sizing it instead to the project's whole work content — room to run every task strictly one after another — is enormous and almost never needed: on a 500-task instance the chain is 503 working ticks against 6708 for the sequential sum, and materialising calendars across the difference was the single largest cost in validating a large project.

That leaves resource contention unaccounted for, deliberately: **a calendar horizon is not a schedule bound.** Contention runs past the chain, sometimes far past it, so anything that schedules widens the horizon first from an actual schedule rather than a formula — the analytic bounds are either far too loose or not bounds at all. Growth stops as soon as it stops buying working time, which is what turns a task whose calendars never agree into a named error rather than an unbounded search, and exhausting a horizon anywhere is reported by name rather than silently truncating a schedule.
